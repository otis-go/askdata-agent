...
class LocalSchemaStore:
    ...
    # TF：词在当前文档出现多少次。
    # IDF：这个词有多稀有。
    # Length Normalization：当前文档是不是特别长。
    # 查询词如果所有候选文档都出现，那么这个词贡献度就低，因为它没有区分度。
    # 查询词在一个文档频繁出现，相对这个文档命中机会变大。
    # 文档的总体长度越长会导致稀释，相对命中程度的影响力变弱。
    def bm25_search(
        self,
        query: str,
        top_k: int,
        allowed_doc_ids: set[str] | None = None, #先过滤再检索 不是每个文档都有权限调用
    ...
    # 分词方法 8.28：
    # 连续英文、数字、下划线作为一个 token；连续中文先整体作为一个 token，
    # 再把这段中文按相邻两个字符做重叠切分。
    @staticmethod
