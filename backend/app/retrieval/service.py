    ...
    def retrieve(
        ...
        *,
        ...
        allowed_doc_ids = (
            {
                document.doc_id
                for document in self.documents      #权限范围
        ...
        # BM25 与向量召回使用同一组检索词。BM25 基于关键词匹配查找相关字段，
        # 返回由“文档索引、BM25 分数”组成的列表（8.26）。
        bm25 = (
    ...
    def _build_raw_documents(self) -> list[dict[str, Any]]:
        ...
        for database in database_names:
            ...
            with self.data_engine.connect(database) as connection:
                for table in (item for item in SCHEMA if item.get("database", "askdata_mock") == database):
