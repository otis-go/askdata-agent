...
router = APIRouter(prefix="/api")  #8.26 HTTP API 入口管理
service = AskDataService()    #8.26  后面负责业务处理的 Service 对象
...
# POST /api/query 同时解析 Request Body 为 QueryRequest payload，并通过
# require_user() 完成认证、取得 AuthUser user，随后二者共同传入 query()（8.26）。
