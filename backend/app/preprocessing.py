...
class PreparedRequest:
    """请求预处理结果。"""
    # 模型 JSON 经 _parse() 校验、规范化以后形成 Python 结构化对象（8.27）。
    # action、confidence 和 reason 决定“接下来去哪”，例如：
    # action = "database_query"、confidence = 0.95、reason = "需要查询新的销售数据"。
    action: RouteName
    confidence: float
    reason: str
    # standalone_query 决定“真正的问题是什么”（8.27）。例如当前 Query 是
    # “那华东呢？”，上一轮是“查询本月各地区销售额”，则 standalone_query 为
    # “查询本月华东地区销售额”。
    standalone_query: str = ""
    rewritten: bool = False
    #数据库查询准备信息 为下一步 Schema Retrieval 准备的搜索意图
