...
class AskDataService:
    ...
    # 进入 Service 时：
    # query = "查询本月各地区销售额"
    # session_id = Route 从 payload.session_id 传来的值
    # workspace = Route 已经 model_dump() 后的 dict / None
    ...
    def submit(
        ...
        normalized = query.strip() #本质是query 去除了多余空格
        access_scope = self.access_controller.resolve(user_id)#user_id=“你是谁” access_scope=“基于你是谁，你能访问什么”
        scoped_session_id = f"{access_scope.user_id}:{session_id}"#session_id 是客户端/Route 带来的原始会话标识；scoped_session_id 是 Service 为用户隔离而生成的内部会话标识。 user-A:demo-session 
        # 如果命中已有待澄清任务，这次 submit() 会提前进入 clarify()，
        # 不会创建新的 task_id，也不会启动新的 workflow.invoke()（8.26）：
        # submit() -> latest_pending(...) -> 有可继续澄清？
        # Yes：self.clarify() 后 return；No：继续创建新 task_id。
        ...
        # session_id 表示一段连续会话；task_id 表示这段会话中的某一次具体任务（8.26）。
        # task_id 也是 Correlation / Trace ID，后续用于 archive、Workflow payload、
        # workflow.invoke(..., task_id) 和失败结果等。
        task_id = uuid.uuid4().hex[:12]
        resolved_workspace = self.context.normalize_workspace(workspace)#把各种可能形态的 workspace 转成 Service 内部统一格式
        ...
        try:
            result = self.workflow.invoke(  #初始状态  从service layer进入workflow layer
                self._payload(
                    task_id,
                    normalized,
                    scoped_session_id,
    ...
    # 截止到这里的数据流（8.26）：
    # POST Body -> payload.query -> AskDataService.submit() 的 query -> query.strip()
    # -> normalized -> self._payload(..., normalized, ...) -> payload["query"]
    # -> self.workflow.invoke(payload, task_id)
    def _payload(
        self,
        task_id: str,
        query: str,
    ...
    ) -> dict[str, Any]:        #initial state 返回内容即是初始状态
